...
需求澄清不是每次变更的固定问答。只有不确定点需要需求方作决定，并满足以下任一条件时，才进入强化澄清：

- 存在多个合理产品方向，选择会明显改变用户体验；
- 关键边界或异常场景尚未明确，遗漏后可能改变验收结果或造成明显返工；
- 选择会实质影响架构、数据、权限或真实外部副作用。

触发强化澄清且环境提供 `$grilling` 技能时，协调者优先使用该技能逐项确认：一次只问一个决策，并提供推荐答案；能从仓库、运行环境或现有规格查明的事实直接调查，不向需求方转问。目标明确、风险局部、只有一个合理答案或可由事实可靠推出的事项不使用 `$grilling`。

当目标、关键规则、范围、边界和验收标准已足以让下游不猜测，强化澄清结束。Requirement Contract 只记录确认结论和依据，不保存冗长问答过程。
