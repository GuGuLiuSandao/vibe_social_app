# M0 Task List: 稳定性基线（2026-03-09 ~ 2026-03-22）

> 版本口径同步（2026-03-13）：
> - 1.0 = 当前基础功能 + 拉黑系统
> - 2.0 = 完整社群系统
> - 3.0 = 动态 / 朋友圈
> 本文档其余里程碑与执行项保持不变。

## 1. 目标与退出条件

M0 目标是把当前“能跑但不稳定”的状态收敛成“可持续迭代”的工程基线。

退出条件（全部满足才算完成）：
- `cd backend && go test ./...` 全绿
- `cd frontend && npm test` 全绿
- 聊天主链路回归通过：登录 -> 建会话 -> 发消息 -> 已读
- 文档与 CI 命令统一（开发者按文档可直接复现）

## 2. 任务清单（按执行顺序）

## T0 基线冻结与问题清单
- [x] 重新执行并保存当前测试失败日志（backend/frontend）
- [x] 产出失败矩阵：失败用例、根因、修复策略、影响范围
- [x] 建立 M0 跟踪表（本文件勾选 + PR 链接）

验收：
- 失败清单可追踪，不再“边修边猜”
- 结果（2026-03-08）：通过
- 说明：失败矩阵已沉淀到 `docs/plans/M0_FAILURE_MATRIX.md`

## T1 修复 Auth 测试与 UID 生成兼容性
- [x] 解决 sqlite 环境 `nextval('user_uid_seq')` 不可用问题（抽象 UID 生成逻辑或按 DB 方言分支）
- [x] 修复 `TestGetCurrentUserWithProtobuf` 中 UID/ID 使用不一致
- [x] 补充注册/登录/鉴权关键路径断言，确保与当前实现一致

涉及文件（预期）：
- `backend/internal/auth/handler.go`
- `backend/internal/auth/handler_test.go`
- `backend/internal/db/database.go`（如需）

验收：
- `go test ./internal/auth -v` 全绿
- 结果（2026-03-08）：通过
- 说明：新增 `nextUserUID()`，Postgres 走 sequence，非 Postgres（含 sqlite）走 `MAX(uid)+1` 回退；修复鉴权测试 UID/ID 混用

## T2 修复 Proto 测试基线
- [x] 将测试读取路径从过时 `*.pb` 文本改为当前 `proto/*.proto`
- [x] 更新过时断言（如旧消息名 `GetConversationsRequest`）到当前协议命名
- [x] 明确只验证“信封边界 + 关键 payload 引用”，避免脆弱字符串断言

涉及文件：
- `backend/internal/proto_test/ws_proto_test.go`

验收：
- `go test ./internal/proto_test -v` 全绿
- 结果（2026-03-08）：通过
- 说明：改为读取 `.proto` 源文件，并更新为 `GetConversationListRequest` 等当前命名

## T3 修复 WebSocket 握手测试与实现一致性
- [x] 对齐缺少 token 的实际行为（当前为 HTTP 401）
- [x] 修复测试中对 content-type / protobuf body 的错误假设
- [x] 增加“无效 token”握手失败用例

涉及文件：
- `backend/internal/websocket/handler_test.go`

验收：
- `go test ./internal/websocket -v` 全绿
- 结果（2026-03-08）：通过
- 说明：握手失败测试改为验证 HTTP 状态与空响应体，并新增 invalid token 场景

## T4 回归与防回退测试补强
- [x] 为“会话创建后 conversation_push”补回归测试（防止再次丢推送）
- [x] 为“私聊发送限制（未互关最多 3 条）”增加服务层测试
- [x] 为“拉黑预留点”写失败用例占位（先红灯，M1 转绿）

涉及文件（预期）：
- `backend/internal/websocket/*_test.go`
- `backend/internal/service/*_test.go`（新增）

验收：
- 新增测试在本地稳定通过，且不依赖手工环境
- 结果（2026-03-08）：通过
- 说明：新增 websocket 回归用例与 service 层限制用例；拉黑预留点以 `Skip` 占位，M1 切换为可执行断言

## T5 命令统一与 CI 校准
- [x] 明确单一验证命令：`go test ./...`、`npm test`
- [x] 校验 Jenkins `Test` 阶段可复现本地结果
- [x] 在 `AGENTS.md`/`README.md` 补充“测试基线要求”

验收：
- 本地与 CI 的测试结果一致
- 结果（2026-03-08）：通过
- 说明：Jenkins `Test` 阶段与本地命令对齐；基线要求已补充到 `AGENTS.md` 与 `README.md`

## T6 手工联调回归脚本
- [x] 按“双账号在线互测”补一份最小回归脚本（步骤 + 预期）
- [x] 覆盖：登录、私聊、群聊、官方话题房、关注关系刷新

建议文档：
- `docs/plans/M0_MANUAL_QA.md`

验收：
- 任意开发者按脚本 20 分钟内完成回归
- 结果（2026-03-08）：通过
- 说明：新增 `docs/plans/M0_MANUAL_QA.md`

## T7 M0 收口与发布准备
- [x] 更新项目状态总览（日期、通过用例、剩余风险）
- [x] 将 M1 输入风险沉淀到 `ROADMAP.md`
- [ ] 打一个可回滚标签（如 `m0-stable`）

验收：
- M0 完成记录可追溯，M1 可直接开工
- 进展（2026-03-08）：前两项已完成，待执行回滚标签

补充记录（2026-03-13）：
- 已建立 `docs/README.md` 文档索引，并开始按 `docs/features/` 新流程落地黑名单功能文档与实现。
- 已启动 2.0 社群系统 1.1 子阶段需求设计，新增目录 `docs/features/community-management-v1-1/`，当前完成 `requirement.md` 初稿，后续进入技术方案阶段。
- `docs/features/community-management-v1-1/technical-solution.md` 已完成首版，方案基于现有 `conversations` / `conversation_participants` 演进群管理能力，并新增申请/邀请模型。

## T8 M1 体验收敛：shadcn/ui 统一与留白优化
- [x] 新增并完成需求与技术方案文档：`docs/features/ui-shadcn-refactor-v1/`
- [x] 前端基础组件统一到 `shadcn/ui`，移除 `vercel-ui` 使用路径
- [x] 登录页、聊天页、群管理组件完成留白与层级优化
- [x] 更新 `AGENTS.md` UI 组件规范，并完成前端回归测试

验收：
- `cd frontend && npm test` 全绿
- 结果（2026-04-04）：通过
- 说明：该任务映射到 `ROADMAP.md` 的 M1「私聊/群聊体验收敛」，已完成 `npm test` 与 `npm run build` 验证

## T9 M1 功能扩展：AI NPC 聊天 MVP
- [x] 新增并完成需求与技术方案文档：`docs/features/ai-npc-chat-mvp-v1/`
- [x] 协议扩展 `ConversationType`：新增 NPC 会话类型
- [x] 后端完成默认 NPC（魔兽酒馆老板）1v1 会话创建与复用
- [x] 后端完成 NPC 自动回复 + OpenAI 兼容 `chat/completions` 接入 + 跨 Session 记忆持久化
- [x] 前端聊天页新增 AI NPC 分类与发起入口
- [x] 完成回归测试并验证不影响既有聊天链路

验收：
- `cd backend && go test ./...` 全绿
- `cd frontend && npm test` 全绿
- 结果（2026-04-06）：通过
- 说明：MVP 暂不开放 NPC 人设自定义与 NPC 多人群聊，已在功能文档中标注后续版本范围；当前支持通过 `LLM_BASE_URL/LLM_API_KEY/LLM_MODEL` 配置外部 `chat/completions`。

## T10 项目健康检查与仓库精简
- [x] 完成全仓结构、依赖、测试、配置和文档审查
- [x] 删除无引用的一次性调试命令、临时产物和未使用前端代码
- [x] 收敛项目状态文档并记录问题分级
- [x] 完成后端测试/静态检查与前端测试/构建验证

验收：
- 不改变现有产品功能、Proto 与数据库结构
- `cd backend && go test ./...`、`go vet ./...` 全绿
- `cd frontend && npm test`、`npm run build` 全绿
- 结果（2026-08-01）：通过
- 说明：对应需求目录 `docs/features/project-health-cleanup-v1/`；同时修复 WebSocket 重连误删新连接、广播锁语义和前端 WS 地址未读取环境变量的问题

## 3. 执行节奏建议（单人 + AI）

Week 1：
- 完成 T0/T1/T2/T3（先把红灯清掉）

Week 2：
- 完成 T4/T5/T6/T7（补强与收口）

每日固定节奏：
- 上午修实现与测试
- 下午全量跑测 + 文档更新
